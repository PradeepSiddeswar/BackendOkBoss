...
exports.create = async (req, res) => {
    ...
    const user = new loginDB({
        ...
        image: "https" + "://" + "3.109.3.181:4000" + "/images/" + req.file.filename,
...
exports.update = (req, res) => {
    ...
    loginDB.findOneAndUpdate({ Coupon: Coupon }, {
        $set: {
            'email': req.body.email,
            'fullname': req.body.fullname,
            'phone': req.body.phone,
        ...
        .then(data => {
            if (!req.body) {
                res.status(400).send({ message: `Can not user with ${id}. May be user not found` })
            } else {
                res.send(data)
            }
        })
